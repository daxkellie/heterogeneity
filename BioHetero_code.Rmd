...
propPost.sem <-
  ...
  summarise(mean = mean(Variance),
            sem = sd(Variance) / sqrt(length(Variance)),
            upper = mean+sem,
            lower = mean-sem)
...
```{r}
# Seed Biomass
modelb_biomass.Seed<- brm(biomass_Seed ~ Legume*Method.comm + 
                            (1|Lab) + (1|CSV) + (1|Replicate), 
                          data = data_CSV,
                          chains = 2, cores = 2)

# Save model
saveRDS(modelb_biomass.Seed, "model_biomassSeed.rds")
```

```{r}
# Load model
modelb_biomass.Seed <- readRDS("model_biomassSeed.rds")

# bayesian R2 estimate of regression model
bayes_R2(modelb_biomass.Seed)
```
...
# we get what we need - for random effect
V_rep <-post$sd_Replicate__Intercept^2 # making it variance
V_site <-post$sd_ProjectName__Intercept^2 + post$sd_ProjectName__z_sagebrush_cover
V_res <- post$sigma^2 
```
...
# we get what we need - for random effect
V_rep <-post$sd_Replicate__Intercept^2 # making it variance
V_site <-post$sd_ProjectName__Intercept^2 + post$sd_ProjectName__z_inv_ann_grass_cover
V_res <- post$sigma^2 
```
...
# we get what we need - for random effect
V_rep <-post$sd_Replicate__Intercept^2 # making it variance
V_site <-post$sd_ProjectName__Intercept^2 + post$sd_ProjectName__z_inv_ann_forb_cover
V_res <- post$sigma^2 
```
...
# Combine
propPost <- as.data.frame(rbind(propPost_nonInv_grass_cover, propPost_nonInv_forb_cover,
