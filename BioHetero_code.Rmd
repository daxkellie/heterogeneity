...
```{r}
# z-transformations: we should get the total variance of 1 
data_BLM %>% 
  mutate(
    z_soil_stab = scale(SoilStability_All), 
    z_non_inv_grass_cover = scale(NonInvPerenGrassCover_AH),
    z_non_inv_forb_cover = scale(NonInvPerenForbCover_AH),
    z_non_inv_shrub_cover = scale(NonInvShrubCover_AH)
    ) %>% 
  select(ProjectName, Replicate, z_soil_stab, z_non_inv_cover) -> sdata_BLM
...
model <- lmer(z_soil_stab ~ z_non_inv_grass_cover + 
                (1 + z_non_inv_grass_cover|ProjectName) + (1 | Replicate), 
              data = sdata_BLM)
summary(model)
hist(resid(model)) # Looks great
```
...
```{r}
# get all posterior samples
post <- posterior_samples(modelb_soil_stab)
names(post)

# we get what we need - for random effect
V_rep <-post$sd_Replicate__Intercept^2 # making it variance
V_site <-post$sd_ProjectName__Intercept^2 + post$sd_ProjectName__z_non_inv_cover
V_res <- post$sigma^2 
```
...
post_mat %>% data.frame %>% mutate(V_all = V_rep + V_site + V_res + V_fix,
                                   ...
                                   P_res = V_res/(V_rep + V_site + V_res + V_fix),
                                   P_fix = V_fix/(V_rep + V_site + V_res + V_fix),
                                   P_all = P_rep + P_site + P_res + P_fix)          ->  post_mat2_soil_stab
...
### Non-Invase Shrub Cover
### Sagebrush Cover
### Invasive Annual Grass Cover
### Invase Annual Forb/Grass Cover
### Total Foliar Cover
...
propPost_soil_stab <- create_bayesVarTable(post_mat2_soil_stab, "soil_stab")
