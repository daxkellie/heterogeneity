...
# How many in PlotKey?
data_BLM %>% 
  group_by(as.factor(PlotID)) %>%
  summarise(no_rows = length(as.factor(PlotID)))
```
...
model0 <- lmer(z_soil_stab ~ z_nonInv_grass_cover + (1 |PlotKey), 
               data = sdata_BLM)
summary(model0)
hist(resid(model0)) 
```
...
```{r}
# finding average proportions of all variables
mean_P_rep <- ((post_mat2_nonInv_grass_cover$P_rep + 
                  post_mat2_nonInv_forb_cover$P_rep + 
                  post_mat2_nonInv_shrub_cover$P_rep + 
                  post_mat2_sagebrush_cover$P_rep + 
                  post_mat2_inv_ann_grass_cover$P_rep + 
                  post_mat2_inv_ann_forb_cover$P_rep + 
                  post_mat2_total_foliar_cover$P_rep) / 7)
...
propPost_BLM <- as.data.frame(cbind(mean_P_rep, mean_P_site, 
