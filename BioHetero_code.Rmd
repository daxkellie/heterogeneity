...
create_VarTable <- function(repOutput, VariableName) {
  
  # retrieve variance values
  varTable <- as.data.frame(repOutput$R)
  
  # make name usable for naming
  ...
  mutate(
    Variable = paste0(VariableName) # Add data identifier column
  ) %>%
...
varTable_plant.NPK <- create_VarTable(rep_plant.NPK, "plant.NPK")
varTable_plant.N <- create_VarTable(rep_plant.N, "plant.N")
varTable_plant.P <- create_VarTable(rep_plant.P, "plant.P")
varTable_plant.K <- create_VarTable(rep_plant.K, "plant.K")

# Create 
varTable_NutNet <- rbind(varTable_plant.NPK, varTable_plant.N, 
...
propPost_plant.NPK <- post_mat2_plant.NPK %>% 
  mutate(
    iteration = 1:length(post_mat2_plant.NPK$P_all), # make a col to represent each test
    Variable = as.factor(paste("plant.NPK"))         # add Variable name
  ) %>%
  select(P_rep, P_site, P_res, P_fix, iteration, Variable) %>%
  pivot_longer(c(-Variable, -iteration), values_to = "Variance") # flip the table

# plant N variance values
propPost_plant.N <- post_mat2_plant.N %>% 
  mutate(
    iteration = 1:length(post_mat2_plant.N$P_all),
    Variable = as.factor(paste("plant.N"))# make a col to represent each test
  ) %>%
  select(P_rep, P_site, P_res, P_fix, iteration, Variable) %>%
  pivot_longer(c(-Variable, -iteration), values_to = "Variance") # flip the table

propPost <- as.data.frame(rbind(propPost_plant.NPK, propPost_plant.N))
...
propPost %>% 
  ...
  stat_summary(fun.y = mean, geom = "bar", position = "stack",width = 0.2, alpha = .6, size = 1) +
