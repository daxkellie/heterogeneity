...
data_NutNet$REPLICLATE <- paste0(data_NutNet$SITE, data_NutNet$TREATMENT)

model <- lmer(plant.NPK ~ TREATMENT + (1|SITE) + (1|REPLICLATE), data = data_NutNet)
summary(model)
...
# Baysian model
modelb <-brm(plant.NPK ~ TREATMENT + (1|SITE) + (1|REPLICLATE), data = data_NutNet,
...
# getting posteior of random effects for each
#blups <- ranef(modelb, summary = F)

# get all posteior samples
...
# we get what we need - for ranomd effect
V_rep <-post$sd_REPLICLATE__Intercept^2 # making it varaince
V_site <-post$sd_SITE__Intercept^2
V_res <- post$sigma^2

# we need to caculate variance for fixed effects - this is not so easy
# get a poistior for fixed effect bits (you need to get this as one - it will be difficult to decompose these 0 why?)
...
# get a posteior of fixed effects
...
post_mat %>% data.frame %>% mutate(V_all = V_rep + V_site + V_res + V_fix,
                                   ...
                                   P_all = P_rep + P_site + P_res + P_fix) %>% tibble ->  post_mat2
