...
pacman::p_load(tidyverse, lme4, lmerTest, MuMIn, rstan, brms, rptR, tidybayes, coda,
               sf, "rnaturalearth", "rnaturalearthdata", modelr, ggthemes, ggmcmc, MCMCglmm, metafor)
...
```{r}
# plant N
model <- lmer(log(plant.N) ~ TREATMENT-1 + (1|SITE) + (1|REPLICATE), data = data_NutNet)
summary(model)
...
#########
data_NutNet$vi <- 1
res <- rma.mv(yi = log(plant.N), V = 1, mods ~ 0 + TREATMENT, random = list(~ TREATMENT | SITE,  ~ 1 | REPLICATE), data = data_NutNet)
summary(res)
...
```{r}
# plant P
model <- lmer(plant.P ~ TREATMENT + (TREATMENT|SITE) + (1|REPLICATE), data = data_NutNet)
summary(model)
hist(resid(model)) # Note that there is an outlier
```
...
data_CSV$treatment <- paste(data_CSV$Legume, data_CSV$Method.comm,sep = "-")

model <- lmer(scale(log(biomass_Seed+1)) ~ treatment - 1+ 
                (treatment|Lab) + (1|CSV) + (1|Replicate), 
                data = data_CSV)
                
summary(model)

model <- lmer(scale(log(biomass_Seed+1)) ~ Legume - 1+ 
                (Legume|Lab) + (1|Replicate), 
                data = data_CSV)
...
```{r}
# finding average proportions of all variables
mean_P_rep <- ((post_mat2_biomass_Seed$P_rep + 
                  post_mat2_biomass_Shoot$P_rep + 
                  post_mat2_biomass_Root$P_rep + 
                  post_mat2_height_legume$P_rep + 
                  post_mat2_N.shoot$P_rep + 
                  post_mat2_C.shoot$P_rep + 
                  post_mat2_substrate_ET$P_rep + 
                  post_mat2_litter$P_rep) / 8)
mean_P_site <- ((post_mat2_biomass_Seed$P_site + 
                  post_mat2_biomass_Shoot$P_site + 
                  post_mat2_biomass_Root$P_site + 
                  post_mat2_height_legume$P_site + 
                  post_mat2_N.shoot$P_site + 
                  post_mat2_C.shoot$P_site + 
                  post_mat2_substrate_ET$P_site + 
                  post_mat2_litter$P_site) / 8)
mean_P_CSV <- ((post_mat2_biomass_Seed$P_CSV + 
                  post_mat2_biomass_Shoot$P_CSV + 
                  post_mat2_biomass_Root$P_CSV + 
                  post_mat2_height_legume$P_CSV + 
                  post_mat2_N.shoot$P_CSV + 
                  post_mat2_C.shoot$P_CSV + 
                  post_mat2_substrate_ET$P_CSV + 
                  post_mat2_litter$P_CSV) / 8)
mean_P_res <- ((post_mat2_biomass_Seed$P_res + 
                  post_mat2_biomass_Shoot$P_res + 
                  post_mat2_biomass_Root$P_res + 
                  post_mat2_height_legume$P_res + 
                  post_mat2_N.shoot$P_res + 
                  post_mat2_C.shoot$P_res + 
                  post_mat2_substrate_ET$P_res + 
                  post_mat2_litter$P_res) / 8)
mean_P_fix <- ((post_mat2_biomass_Seed$P_fix + 
                  post_mat2_biomass_Shoot$P_fix + 
                  post_mat2_biomass_Root$P_fix + 
                  post_mat2_height_legume$P_fix + 
                  post_mat2_N.shoot$P_fix + 
                  post_mat2_C.shoot$P_fix + 
                  post_mat2_substrate_ET$P_fix + 
                  post_mat2_litter$P_fix) / 8)


propPost_CSV <- as.data.frame(cbind(mean_P_rep, mean_P_site, 
                                    mean_P_CSV, mean_P_res, mean_P_fix))
```
...
plot2_CSV <- propPost_CSV %>% mutate(
  ...
  stat_summary(fun.y = mean, 
               ...
               geom = "errorbar",
               width = 0.00, position = position_dodge(width = 0.90), colour = "black") +
  scale_y_continuous(labels = scales::percent) +
  labs(x = "Outcome Variables",
       y = "Proportion of Total Variance") +
  scale_fill_ptol() + scale_colour_ptol() + coord_flip(ylim = c(0,1)) +
...
# Each row of data is a replicate, so this may not be necessary

data_BLM$Replicate <- paste0(data_BLM$ProjectName, data_BLM$PlotID)


# How many replicates in ProjectName?
data_BLM %>% 
  group_by(as.factor(ProjectName)) %>%
  summarise(no_rows = length(as.factor(ProjectName)))
```
...
data_BLM %>% 
  mutate(
    ...
    PlotKey = as.factor(PlotKey)
    ) %>% 
  select(ProjectName, PlotID, Replicate, z_soil_stab, z_nonInv_grass_cover,
...
model0 <- lmer(z_soil_stab ~ z_nonInv_grass_cover + (1 |ProjectName), 
               data = sdata_BLM)
summary(model0)
hist(resid(model0)) 

# difference between random slope and random intercept model is pretty much negligible, but every so slightly more variance is attributed to the random slope variable
```
...
model <- lmer(z_soil_stab ~ z_nonInv_grass_cover + 
                (1 + z_nonInv_grass_cover|ProjectName), 
              data = sdata_BLM)
summary(model)
hist(resid(model)) # Looks great
```
...
# mcmc object (from coda package)
post_mat <- as.mcmc(data.frame(V_site, V_res, V_fix, row.names = NULL))
...
post_mat %>% 
  data.frame %>% 
  mutate(V_all = V_site + V_res + V_fix,
         # P_rep = V_rep/(V_rep + V_site + V_res + V_fix),
         P_site = V_site/(V_site + V_res + V_fix),
         P_res = V_res/(V_site + V_res + V_fix),
         P_fix = V_fix/(V_site + V_res + V_fix),
         P_all = P_site + P_res + P_fix)          ->  post_mat2_nonInv_grass_cover
...
model <- lmer(z_soil_stab ~ z_nonInv_forb_cover + 
                (1 + z_nonInv_forb_cover|ProjectName), 
              data = sdata_BLM)
summary(model)
hist(resid(model)) # Looks great
```
...
post_mat %>% 
  data.frame %>% 
  mutate(V_all = V_site + V_res + V_fix,
         # P_rep = V_rep/(V_site + V_res + V_fix),
         P_site = V_site/(V_site + V_res + V_fix),
         P_res = V_res/(V_site + V_res + V_fix),
         P_fix = V_fix/(V_site + V_res + V_fix),
         P_all = P_site + P_res + P_fix)          ->  post_mat2_nonInv_forb_cover
...
model <- lmer(z_soil_stab ~ z_nonInv_shrub_cover + 
                (1 + z_nonInv_shrub_cover|ProjectName), 
              data = sdata_BLM)
summary(model)
hist(resid(model)) # Looks great
```
...
post_mat %>% 
  ...
  mutate(V_all = V_site + V_res + V_fix,
         ...
         P_res = V_res/(V_site + V_res + V_fix),
         P_fix = V_fix/(V_site + V_res + V_fix),
         P_all = P_site + P_res + P_fix)          ->  post_mat2_nonInv_shrub_cover
...
model <- lmer(z_soil_stab ~ z_sagebrush_cover + 
                (1 + z_sagebrush_cover|ProjectName), 
              data = sdata_BLM)
summary(model)
hist(resid(model)) # Looks great
```
...
post_mat %>% 
  ...
  mutate(V_all = V_site + V_res + V_fix,
         ...
         P_res = V_res/(V_site + V_res + V_fix),
         P_fix = V_fix/(V_site + V_res + V_fix),
         P_all = P_site + P_res + P_fix)          ->  post_mat2_sagebrush_cover
...
model <- lmer(z_soil_stab ~ z_inv_ann_grass_cover + 
                (1 + z_inv_ann_grass_cover|ProjectName), 
              data = sdata_BLM)
summary(model)
hist(resid(model)) # Looks great
```
...
post_mat %>% 
  ...
  mutate(V_all = V_site + V_res + V_fix,
         ...
         P_res = V_res/(V_site + V_res + V_fix),
         P_fix = V_fix/(V_site + V_res + V_fix),
         P_all = P_site + P_res + P_fix)          ->  post_mat2_inv_ann_grass_cover
...
model <- lmer(z_soil_stab ~ z_inv_ann_forb_cover + 
                (1 + z_inv_ann_forb_cover|ProjectName), 
              data = sdata_BLM)
summary(model)
hist(resid(model)) # Looks great
```
...
post_mat %>% 
  ...
  mutate(V_all = V_site + V_res + V_fix,
         ...
         P_res = V_res/(V_site + V_res + V_fix),
         P_fix = V_fix/(V_site + V_res + V_fix),
         P_all = P_site + P_res + P_fix)          ->  post_mat2_inv_ann_forb_cover
...
model <- lmer(z_soil_stab ~ z_total_foliar_cover + 
                (1 + z_total_foliar_cover|ProjectName), 
              data = sdata_BLM)
summary(model)
hist(resid(model)) # Looks great
```
...
post_mat %>% 
  ...
  mutate(V_all = V_site + V_res + V_fix,
         ...
         P_res = V_res/(V_site + V_res + V_fix),
         P_fix = V_fix/(V_site + V_res + V_fix),
         P_all = P_site + P_res + P_fix)          ->  post_mat2_total_foliar_cover
...
```{r}
# finding average proportions of all variables
mean_P_site <- ((post_mat2_nonInv_grass_cover$P_site + 
                  post_mat2_nonInv_forb_cover$P_site + 
                  post_mat2_nonInv_shrub_cover$P_site + 
                  post_mat2_sagebrush_cover$P_site + 
                  post_mat2_inv_ann_grass_cover$P_site + 
                  post_mat2_inv_ann_forb_cover$P_site + 
                  post_mat2_total_foliar_cover$P_site) / 7)
mean_P_res <- ((post_mat2_nonInv_grass_cover$P_res + 
                  post_mat2_nonInv_forb_cover$P_res + 
                  post_mat2_nonInv_shrub_cover$P_res + 
                  post_mat2_sagebrush_cover$P_res + 
                  post_mat2_inv_ann_grass_cover$P_res + 
                  post_mat2_inv_ann_forb_cover$P_res + 
                  post_mat2_total_foliar_cover$P_res) / 7)
mean_P_fix <- ((post_mat2_nonInv_grass_cover$P_fix + 
                  post_mat2_nonInv_forb_cover$P_fix + 
                  post_mat2_nonInv_shrub_cover$P_fix + 
                  post_mat2_sagebrush_cover$P_fix + 
                  post_mat2_inv_ann_grass_cover$P_fix + 
                  post_mat2_inv_ann_forb_cover$P_fix + 
                  post_mat2_total_foliar_cover$P_fix) / 7)


propPost_BLM <- as.data.frame(cbind(mean_P_site, 
                                    mean_P_res, mean_P_fix))
```
...
create_bayesVarTable <- function(posteriorVarianceDF, variableName) {
  ...
  select(P_site, P_res, P_fix, iteration, Variable) %>%
  pivot_longer(c(-Variable, -iteration), values_to = "Variance") # flip the table
}
...
propPost_BLM <- propPost_BLM %>%
    ...
    Variable = as.factor(recode(Variable,
       ...
       arrange_at(4, desc) %>%
     arrange(match(name, c("Location", "Residual", "Fixed")))
...
plot1_BLM <- 
  propPost_BLM.sem2 %>% mutate(
    Component = factor(name, levels=c("Fixed", "Residual", "Location"))) %>% 
...
plot2_BLM <- propPost_BLM %>% mutate(
  name = factor(name, levels=c("Fixed", "Residual", "Location"))) %>% 
